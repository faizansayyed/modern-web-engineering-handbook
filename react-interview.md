...
- [Interview Method](#-how-to-use-this-guide-in-an-interview)
- [React Hooks](#-beginner--mid-level)
- [React Rendering & Core Concepts](#react-rendering--core-concepts)
- [Redux & Cross-Tab Synchronization](#-architecture--design)
- [Testing](#testing)
- [Real-Time Communication](#real-time-communication)

> **Goal:** Use this guide to answer interviews with a clear **definition → why → flow → production scenario → trade-off** structure.
...
> **Interview rule:** For each hook, remember **what it does + one real use case**.

### `useState`

**Meaning:** Stores local state and re-renders the component when the state changes.

**Use case:** Input, modal, tab, toggle.
...
**Meaning:** Manages related/complex state through actions and a reducer.

**Use case:** Multi-step form or complex UI state transitions.
...
**Meaning:** Reads shared values without passing props through every component.

**Use case:** Theme, locale, authenticated user, configuration.
...
**Meaning:** Stores a value across renders without causing a re-render.

**Use case:** DOM element, timer ID, previous value.
...
**Meaning:** Controls what a parent can access through a child's ref.

**Use case:** Expose a custom `focus()` or `open()` method from a reusable component.
...
**Meaning:** Synchronizes a component with an external system.

**Use case:** API subscription, timer, WebSocket, browser event.
...
> **Rule:** If you are only deriving data from existing React state/props, you usually don't need an Effect.

---

### `useEffectEvent`

**Meaning:** Lets Effect logic read the latest props/state without making that logic reactive.

**Use case:** A WebSocket should reconnect only when the URL changes, while its event handler still sees the latest state.

```tsx
const onMessage = useEffectEvent((msg) => {
  console.log(latestUser, msg);
});
```
...
**Meaning:** Runs after the DOM is updated but before the browser paints.

**Use case:** Measure an element or position a tooltip without visible flicker.
...
**Meaning:** Runs very early so CSS-in-JS libraries can insert styles before layout effects.

**Use case:** Mainly library/framework internals; rarely needed in normal application code.

```tsx
useInsertionEffect(() => {
  // Usually used by CSS-in-JS libraries
}, []);
```
...
**Meaning:** Caches the result of a calculation until dependencies change.

**Use case:** Expensive filtering/sorting of a large dataset.
...
> **Don't use it just because a calculation exists—use it when memoization provides a benefit.**

---

### `useCallback`

**Meaning:** Caches a function reference until dependencies change.

**Use case:** Passing a callback to a memoized child.
...
**Meaning:** Marks a state update as non-urgent so urgent interactions stay responsive.

**Use case:** User typing should stay fast while an expensive result list updates.
...
**Meaning:** Creates a lower-priority version of a value so expensive UI can lag behind urgent UI.

**Use case:** Keep a search input responsive while a large result list renders.

```tsx
const deferredSearch = useDeferredValue(search);
```

> **Easy difference:** `useTransition` defers an **update**; `useDeferredValue` defers a **value**.

---

### `useSyncExternalStore`

**Meaning:** Safely subscribes React to state that lives outside React.

**Use case:** Custom external store, browser API, or third-party state library.
...
**Meaning:** Manages the result and pending state of an async Action.

**Use case:** Form submission where you need result + pending/error state.
...
**Meaning:** Shows the expected result immediately while the server operation is pending.

**Use case:** Add a comment to the UI immediately before the API confirms it.
...
**Meaning:** Reads the submission status of the nearest parent `<form>`.

**Use case:** Disable a submit button while a form Action is running.
...
**Babel** is a JavaScript compiler/transpiler. It transforms source code such as modern JavaScript and JSX into JavaScript that your target browsers can execute. It is a build-time tool, not a runtime library.
Your Code
|
v
React JSX
...
**`.eslintrc.json`** is an ESLint configuration file. It defines static-analysis rules that catch problems TypeScript alone may not catch, such as incorrect React Hook usage, accessibility issues, and project-specific code-quality rules.


**`.prettierrc`** contains Prettier formatting rules. It keeps code style consistent across the team by automatically applying the same formatting conventions.


## ⚛️ React Core Concepts

**Synthetic Events** are React’s event abstraction over browser events. They provide a consistent event API such as `onClick={(e) => console.log(e.target)}` while React manages event handling through its event system. Modern React no longer uses the old pooled-event system.
...
## 🌐 Browser & Routing Concepts

**Shadow DOM** is a browser-native DOM encapsulation mechanism: it creates an isolated DOM subtree and can scope styles to a component. **Virtual DOM** is a JavaScript representation of UI that React uses during reconciliation to determine what DOM changes are needed. They solve different problems.
...
For any React/frontend question, try to answer in this order:

1. **Definition** — What is it?
2. **Problem solved** — Why does it exist?
3. **When to use** — Give a practical scenario.
4. **How it works** — Explain the lifecycle or data flow.
5. **Production concerns** — Mention performance, security, error handling, testing, or observability where relevant.
6. **Trade-off** — Explain when you would choose an alternative.

> **Senior-level answer:** Avoid stopping at “what it is.” Show that you understand **why it exists, when to use it, what can go wrong, and how you would operate it in production**.
